...
const Home = () => {
  ...
  function postUser() {
    fetch(`https://playground.4geeks.com/todo/users/${myUser}`, {
      ...
      .then((response) => {
        if (!response.ok) {
          throw new Error(
            `${response.status}, ${response.statusText}, User already exists.`
          );
        }
      })
      .catch((error) => {
  ...
  return (
    <div
      className="container d-flex justify-content-center flex-column p-3 
		my-5 bg-warning-subtle shadow fuzzy-bubbles-regular rounded"
    >
      <h1 className="text-center text-color-a ">
        <b>To Do List</b>
      </h1>
      <ul className="list-group w-100 align-center ">
        ...
        {arrayToDos.map(({ label, is_done, id }, index, array) => (
          <li
            className={`list-group-item d-flex list-group-item-action 
				align-items-center show-hiden list-group-item-warning
          ...
          >
            ...
            />
            <label className="form-check-label align-items-center">
              {label}
            </label>
        ...
        <li className="list-group-item border-0 bg-a d-flex align-items-center ">
          {`${arrayToDos.length} task${
            arrayToDos.length !== 1 ? "s" : ""
          } left`}
          <button
            className="btn btn-outline-warning border-0 btn-sm ms-auto"
            onClick={postUser}
          >
